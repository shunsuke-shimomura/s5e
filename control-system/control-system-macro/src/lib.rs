...
pub fn derive_estimation_state(input: TokenStream) -> TokenStream {
    ...
    for field in fields_named.iter() {
        ...
        // --- シグマ点 struct のフィールド型 ---
        let sigma_ty = quote! { <#ty as ::control_system::components::GaussianValueType>::Sigma };
        ...
        // --- ノミナル struct のフィールド型 ---
        let nominal_ty =
            quote! { <#ty as ::control_system::components::GaussianValueType>::Nominal };
        ...
        let sigma_component_dim =
            quote! { <#sigma_ty as ::control_system::components::GaussianSigmaType>::DIM };
        ...
        write_blocks.push(quote! {
            {
                let dim = <#sigma_ty as ::control_system::components::GaussianSigmaType>::DIM;
                <#sigma_ty as ::control_system::components::GaussianSigmaType>::write_to_slice(
                    &value.#ident,
        ...
        read_blocks.push(quote! {
            let #ident = {
                let dim = <#sigma_ty as ::control_system::components::GaussianSigmaType>::DIM;
                let v = <#sigma_ty as ::control_system::components::GaussianSigmaType>::read_from_slice(
    ...
    let expanded = quote! {
        ...
        impl ::control_system::value_structs::NominalStructTrait for #nominal_name {
        ...
        impl ::control_system::value_structs::ValueStructTrait for #name {
            type SigmaStruct = #sigma_name;
        ...
        impl ::control_system::value_structs::OutputStructTrait<{ #dim_expr }> for #name {
            fn error_from(
                &self,
                measured: &Self,
            ) -> Self::SigmaStruct {
        ...
        impl ::control_system::value_structs::StateStructTrait<{ #dim_expr }> for #name {

            fn to_sigma(&self, covariance: ::nalgebra::SMatrix::<f64, { #dim_expr }, { #dim_expr }>) -> Result<::control_system::value_structs::SigmaPointSetWithoutWeight<Self, { #dim_expr }>, ::control_system::components::KalmanFilterError>
            where
                Self: Sized,
            {
                ::control_system::ukf::generate_sigma_points(self, covariance)
            }
        }
...
pub fn derive_estimation_gaussian_input(input: TokenStream) -> TokenStream {
    ...
    for (group_name, fields) in groups.iter() {
        if let Some(group) = group_name {
            ...
            for field_info in fields.iter() {
                let ty = &field_info.ty;
                let sigma_ty =
                    quote! { <#ty as ::control_system::components::GaussianValueType>::Sigma };
                let sigma_component_dim =
                    quote! { <#sigma_ty as ::control_system::components::GaussianSigmaType>::DIM };
    ...
    let expanded = quote! {
        ...
        impl ::control_system::value_structs::GaussianInputTrait<{ #dim_expr }> for #gaussian_name {
            type SigmaStruct = #sigma_name;
            type MeanStruct = #name;
            fn to_sigma(&self) -> Result<::control_system::value_structs::SigmaPointSetWithoutWeight<Self::MeanStruct, { #dim_expr }>, ::control_system::components::KalmanFilterError>
            where
                Self: Sized
            {
                let mut covariance =
                ...
                let sqrt_cov = covariance
                    .cholesky()
                    .ok_or(::control_system::components::KalmanFilterError::CholeskyDecompositionFailed)?
                    .l();
                let mean = self.mean();
                ::control_system::ukf::generate_sigma_points_from_sqrt_covariance(&mean, sqrt_cov)
            }
